...
import 'package:my_project_name/features/eCommerce/presentation/pages/add_page.dart';
...
void main() {
  ...
  testWidgets('shows success Snackbar on successful product creation',
    ...
    expect(find.text('The Product is created Successfully'), findsOneWidget);
  });
  ...
  testWidgets('shows error Snackbar on product creation failure',
    ...
    whenListen(
      ...
      Stream<ProductState>.fromIterable(
          [const ErrorState(message: 'Failed to create a product')]),
    );
    ...
    expect(find.text('Failed to create a product'), findsOneWidget);
  });
}
